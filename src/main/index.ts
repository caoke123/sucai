...
[TASK 1] title: ≤60字中文标题 | shortTitle: ≤10字 |
         ...
         description: 2-3句中文卖点 | material: 严格从材质白名单复制1个原文值，禁止自造
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        // ===== 第3层：核心视觉任务（图片全部就位后再发指令）=====
        const skipBasic = payload.skipBasicInfo === true
        const skuCount = payload.skuIds.length

        const visualTaskPrompt = `[PRODUCT CONTEXT]
...
你是跨境电商选品专家。你已经看到了主图（产品整体）和 ${skuCount} 张 SKU 单体图。请在视觉分析完成后，严格按下方规则输出纯 JSON（不带 Markdown 代码块）。

【Visual Counter 校验 —— 必须核对】
本消息共包含 ${skuCount} 张 SKU 单体图片。最终输出的 skus 数组必须严格包含 ${skuCount} 个元素，与 SKU 图片数量一一对应，严禁合并或遗漏。

${skipBasic ? `[预分析已完成，基础信息已填写，只需完成 SKU 识别]
` : `[TASK 1 - 基础信息]
title: ≤60字中文标题 | shortTitle: ≤10字 | category: 包包挂件/手机挂件/车内配饰/毛绒玩具 | description: 2-3句中文卖点描述

[TASK 2 - 属性识别]
material: 严格从本文末尾 [材质白名单] 中完整复制1个原文值填入，禁止自造、缩写、翻译或近似匹配。例如白名单中有"PU革"则必须填"PU革"，不能填"合成革"或"人造革"
pattern: 1-3个英文单词 Title Case，描述产品外观造型，非颜色

[TASK 4 - Shopee]
shopee.title: 纯英文标题，严格120-160字符（含空格）。从本文末尾 [关键词库] 中选3-5个最相关的词自然融入
shopee.descriptionText: 纯英文纯文本，六段结构[PRODUCT NAME]/[SPECIFICATIONS]/[USE SCENARIOS]/[DESCRIPTION]/[HOW TO USE]/[CARE INSTRUCTIONS]，每段标题全大写方括号，段间空一行
shopee.material: 与material字段一致

`}[TASK 3 - SKU 识别]
每个SKU根据对应的单体图片生成:
- skuName: 2-10汉字，颜色/款式特征，各SKU不重复
- skuNameEn: 2-5词 Title Case，≤28字符
中文名已确定的SKU: skuName照用原值不修改，只生成skuNameEn

【IMPORTANT — skuId 回填规则】
每个 SKU 图的 skuId 必须使用上文 SKU_ID: 标签标注的原始完整路径，一字不改地回填。
例如上文标注了 SKU_ID: ${(payload.skuIds[0] || 'D:/path/image.jpg').replace(/\\/g, '/')}，则 skuId 必须填 "${(payload.skuIds[0] || 'D:/path/image.jpg').replace(/\\/g, '/')}"，不得修改、截断或编造。

${skipBasic ? `输出格式:
{"skus":[{"skuId":"${(payload.skuIds[0] || '').replace(/\\/g, '/')}","skuName":"茱萸粉毛球款","skuNameEn":"Rose Pink Charm"}]}` : `输出格式:
{ "title":"...", "shortTitle":"...", "category":"...", "description":"...", "material":"...", "pattern":"...", "shopee":{"title":"...","descriptionText":"...","material":"..."}, "skus":[{"skuId":"${(payload.skuIds[0] || '').replace(/\\/g, '/')}","skuName":"茱萸粉毛球款","skuNameEn":"Rose Pink Charm"}] }`}

 输出前确认: skus数组长度=${skuCount}无遗漏; skuId必须原样回填SKU_ID标签值; skuNameEn ≤28字符; material必须从材质白名单完整复制原文不许缩写翻译或自造; shopee.material与material完全一致; 纯JSON无代码块`

        contentParts.push({ type: 'text', text: visualTaskPrompt })

        // ===== 第4层：参考数据（附录，不影响视觉识别）=====
        contentParts.push({
          type: 'text',
          text: `[材质白名单 — 必须从此列表中选1个最匹配的值填入material字段]
["竹纤维","帆布","羊绒","棉","羊毛","尼龙","涤纶","人造丝","PVC","橡胶","硅胶","丝绒","ABS","粘土","纸","塑料","布面","木材","泡沫","玻璃","皮革","金属","雪纺","牛仔布","毡","皮毛","针织","蕾丝","亚麻","其他","丝绸","合成皮","纺织","毛圈","莱卡","人造棉","Voal","钻石","玉","银","珍珠","钢","编织","PU革","网状布","绒革","聚碳酸酯纤维","铝","陶瓷的","铜","不锈钢","涂层的","合金","平织布","热塑性弹性体","纸板","太空棉","亚克力","结石","精梳棉","法兰绒","超细纤维","黄麻","乙烯基胶","黄铜","胶乳","记忆泡沫","羽毛","聚酯纤维填充","碳素钢","羊皮","热塑性聚氨酯","玛瑙","水晶","Modal","Faux Fur","Corduroy","Braid","Calfskin","Cowhide","Goatskin","Lambskin","Metallic","Satin","Twill","Acetate","Non-woven","Polypropylene","Recycled","Aluminum Alloy","Artificial Leather","Shell","Silk/Satin","Straw/Bamboo","Polypropylene (PP)","Polyethylene (PE)","Polystyrene (PS)","Stainless","Gauze","Mineral","Non-woven Fabric","Chenille","Turquoise","Resin","Iron","Polycarbonate (PC)","Cordura","Polímero","Polycotton","Cotton Blend","Cotton Polyester","Cotton Linen","Silk Satin","Batik Fabric","Synthetic Fabric","Knitted Fabric","Waterproof Fabric","Stretch Fabric","Organza","Plush","Genuine Leather","Microfiber Leather","Vegan Leather","Zinc Alloy","Enamel","Rhinestone","Imitation Pearl","Beads","毛绒"]

[关键词库 — Shopee标题中选3-5个最相关的自然融入]
["handbag charm","croissant bag charm","y2k plush bunny keychain","dice keychain","my melody keychain","pink keychain","bag designs accessories","brown keychain for bag aesthetic","snoopy keychain plush","bunny keychain","cute keychain","snoopy stuffed toy","palawit sa bag","keychain cute","cinnamoroll","turtle stuffed toy","bag accessories","keychain accessories","accessories for bag","bag accessories keychains and pins","bag keychain aesthetic","chiikawa miniso","cherry bag charm","8 ball keychain","bag chain accessories","red bag charm","cherry keychain coach","bag keychain accessories","keychain cute for bag","coach cherry charm","keychains","bag charms","coach cherry keychain","cherry charm","key chain","bag keychain","snoopy plushie","key chain holder","keychain cherry","bag charms accessories","turtle keychain","keychain for bag","bag accessories charms","bag charm","snoopy keychain","snoopy","keychain for bag aesthetic","cherry keychain","keychain","cherry keychain for bag"]`,
        })
        ...
        ;(async () => {
          try {
            while (true) {
              ...
              // 处理最后一帧数据（done=true 时 value 可能仍携带数据）
              if (value) {
          ...
          try {
            ...
            console.log('[AI] JSON parsed OK, skus count:', (parsed as Record<string,unknown>).skus ? (parsed as Record<string,unknown>).skus!.length : 0)
